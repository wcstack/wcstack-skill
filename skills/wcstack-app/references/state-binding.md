...
31. `wcs-validate --strict` exits `1` on **warnings** too (severities are unchanged; only the exit threshold moves). It is the way to make a path typo (`wcs/binding-path-missing`, a warning) fail CI — but run it only once every `<wcs-state src>` resolves relative to its HTML, because an unresolvable external state leaves warnings that now fail the build. `--errors-only --strict` keeps the output quiet while still failing.
