...
Content verified against **wcstack v2.0.0** (READMEs, examples, and source as of 2026-09). v2 is the first breaking release: **one state tree per rootNode**, extended with mounts. `<wcs-state name>` and `path@name` are gone — the migration table is in `references/state-binding.md` §2. If the installed/CDN version is much newer, spot-check syntax against the package READMEs.
...
| `<wcs-router focus="...">` with any value other than `heading` (empty, or a typo) | On 2.0.0 the router still passes `focusReset: "manual"`, so the browser's default focus reset is disabled and nothing replaces it — focus stays on the old screen after every navigation | Write exactly `focus="heading"`, or omit the attribute entirely |
| `<wcs-router focus="heading">` on a route whose content has no heading | On 2.0.0 focus is left on the clicked element (a persistent nav link), i.e. on the previous screen — a screen reader never enters the new content | Give every route a visible heading, near the top of its content (scroll-to-top wins over the focus scroll) |
